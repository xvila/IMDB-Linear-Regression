...
class MovieSpider(scrapy.Spider):
    ...
    def parse_movie(self,response): 
        ...
        try:
            ...
            director = None
        try:
            meta_rating = response.xpath('//div[@class="metacriticScore score_favorable titleReviewBarSubItem"]/span/text()').extract()[0]
        except:
            meta_rating = None
        try:
            user_rating_count = response.xpath('//span[@itemprop="ratingCount"]/text()').extract()[0]
        except:
            user_rating_count = None
        try:
            opening = response.xpath('//div[@class="txt-block"]/h4/text()[contains(.,"Opening Weekend")]/../../text()').extract()[1].strip()
        except:
            opening = None
